...
(function () {
  ...
  const defaultState = () => ({
    ...
    brief: {
      ...
      speakers: [{ name: '', title: '', org: '', linkedin: '', notes: '', chair: false }, { name: '', title: '', org: '', linkedin: '', notes: '', chair: false }, { name: '', title: '', org: '', linkedin: '', notes: '', chair: false }],
    },
    mode: 'ai',
    result: null,
    aiUsed: false,
  ...
  const Generator = {
    ...
    loadForEdit(id) {
      ...
      s.brief.speakers = (c.speakers || []).map((sp) => ({ name: sp.name, title: sp.title || '', org: sp.org || '', linkedin: sp.linkedin || '', notes: (sp.expertise || []).join(', '), chair: (c.chairs || []).indexOf(sp.id) >= 0 }));
    ...
    step2() {
      ...
      const row = (s, i) => '<div class="speaker-row" data-i="' + i + '"><div><label>Full name</label><input data-k="name" value="' + esc(s.name) + '" placeholder="e.g. Amara Okafor"></div><div><label>Title</label><input data-k="title" value="' + esc(s.title) + '" placeholder="Chief Compliance Officer"></div><div><label>Organisation</label><input data-k="org" value="' + esc(s.org) + '" placeholder="Northgate Bank"></div><div><label>LinkedIn profile (optional)</label><input data-k="linkedin" value="' + esc(s.linkedin) + '" placeholder="linkedin.com/in/…"></div><div><button class="btn btn-ghost btn-sm" type="button" data-remove="' + i + '" aria-label="Remove speaker">Remove</button></div><div class="sp-notes"><label>Expertise or notes (optional)</label><input data-k="notes" value="' + esc(s.notes) + '" placeholder="AML, sanctions; former regulator; happy to keynote"></div><label class="sp-chair" title="Chairs open and close the programme and host the welcome reception"><input type="checkbox" data-k="chair"' + (s.chair ? ' checked' : '') + '><span>Programme chair</span></label></div>';
      html('#wizard', '<div class="panel"><h2>Speakers</h2><p class="muted">Add the speakers you have confirmed or invited. Tick <strong>Programme chair</strong> for anyone who should open and close the programme; leave it off and the planner proposes chairs from the rest of the faculty while your speakers keep the keynote and panel slots. LinkedIn profiles are stored and linked from each speaker page. You can also generate a programme with no speakers and add them later.</p>' +
        '<div id="rows">' + b.speakers.map(row).join('') + '</div><p class="small muted mt-1" id="chair-note"></p>' +
      ...
      const chairNote = () => {
        const names = b.speakers.filter((s) => s.chair && s.name.trim()).map((s) => s.name.trim());
        $('#chair-note').innerHTML = names.length
          ? '<strong>Programme chair' + (names.length > 1 ? 's' : '') + ':</strong> ' + esc(names.join(', ')) + '. They open and close the programme and host the welcome reception.'
          : '<strong>No programme chair selected.</strong> ' + (b.fillFaculty !== false ? 'The planner will propose placeholder chairs, shown as “Invited” on the site, for you to replace as you confirm them.' : 'The opening and closing sessions will show chairs to be confirmed.');
      };
      const read = () => { b.speakers = $$('.speaker-row').map((r) => ({ name: $('[data-k="name"]', r).value, title: $('[data-k="title"]', r).value, org: $('[data-k="org"]', r).value, linkedin: $('[data-k="linkedin"]', r).value, notes: $('[data-k="notes"]', r).value, chair: $('[data-k="chair"]', r).checked })); b.fillFaculty = $('#fill-faculty').checked; b.facultyTarget = Math.max(1, Math.min(40, +$('#faculty-target').value || defaultTarget)); Generator.save(); chairNote(); };
      chairNote();
      $('#fill-faculty').addEventListener('change', read); $('#faculty-target').addEventListener('input', read);
      $('#rows').addEventListener('input', read); $('#rows').addEventListener('change', read);
      $('#rows').addEventListener('click', (e) => { const btn = e.target.closest('[data-remove]'); if (!btn) return; read(); b.speakers.splice(+btn.dataset.remove, 1); if (!b.speakers.length) b.speakers.push({ name: '', title: '', org: '', linkedin: '', notes: '', chair: false }); Generator.save(); Generator.render(); });
      $('#add').addEventListener('click', () => { read(); b.speakers.push({ name: '', title: '', org: '', linkedin: '', notes: '', chair: false }); Generator.save(); Generator.render(); });
      $('#paste-toggle').addEventListener('click', () => $('#paste').classList.toggle('hide'));
      $('#paste-apply').addEventListener('click', () => { read(); const lines = $('#paste-text').value.split('\n').map((l) => l.trim()).filter(Boolean); lines.forEach((l) => { const p = l.split('|').map((x) => x.trim()); if (p[0]) b.speakers.push({ name: p[0], title: p[1] || '', org: p[2] || '', linkedin: p[3] || '', notes: p[4] || '', chair: /^(chair|yes|y|true)$/i.test(p[5] || '') }); }); b.speakers = b.speakers.filter((s) => s.name.trim()); if (!b.speakers.length) b.speakers.push({ name: '', title: '', org: '', linkedin: '', notes: '', chair: false }); Generator.save(); Generator.render(); });
      $('#back').addEventListener('click', () => Generator.go(1));
      $('#next').addEventListener('click', () => { read(); b.speakers = b.speakers.filter((s) => s.name.trim()); if (!b.speakers.length) b.speakers.push({ name: '', title: '', org: '', linkedin: '', notes: '', chair: false }); Generator.save(); Generator.go(3); });
    },
    ...
    briefForEngine() {
      ...
      return {
        ...
        suppliedChairs: false, // chairs are only the speakers ticked as Programme chair
        speakers: b.speakers.filter((s) => s.name.trim()).map((s) => ({ name: s.name, title: s.title, org: s.org, linkedin: s.linkedin || null, supplied: true, chair: !!s.chair, expertise: s.notes ? s.notes.split(/[,;]/).map((x) => x.trim()).filter(Boolean) : [] })),
      };
    },
    ...
    async runAI(conf, log, tick) {
      ...
      const roster = conf.speakers.map((s) => ({ id: s.id, name: s.name, title: s.title, org: s.org, expertise: s.expertise, supplied: !!s.supplied, proposed: !!s.proposed, chair: (conf.chairs || []).indexOf(s.id) >= 0 }));
    ...
    step4() {
      ...
      html('#wizard',
        ...
        '<div class="panel"><div class="preview-tabs" role="tablist">' + ['Overview', 'Programme', 'Speakers', 'Venue & hotels', 'Partnership', 'Registration', 'JSON'].map((t, i) => '<button role="tab" class="' + (i === 0 ? 'active' : '') + '" data-tab="' + i + '">' + t + '</button>').join('') + '</div>' +
          ...
          '<div class="preview-pane" data-pane="2">' + ((c.speakers || []).length ? '<p class="small muted">' + c.speakers.filter((s) => s.supplied).length + ' supplied · ' + c.speakers.filter((s) => s.proposed).length + ' proposed placeholders (shown as “Invited” on the site) · ' + c.speakers.filter((s) => s.researched).length + ' with researched biographies' + ((c.chairs || []).length ? ' · chaired by ' + esc((c.chairs || []).map((id) => (spk(id) || {}).name).filter(Boolean).join(', ')) : ' · no programme chair') + '</p><div class="grid grid-2">' + c.speakers.map((s) => '<div class="speaker-card" style="cursor:default">' + UI.avatar(s) + '<span><span class="name">' + esc(s.name) + '</span><span class="role" style="display:block">' + esc(s.title) + (s.org ? ', ' + esc(s.org) : '') + '</span><span style="display:flex;gap:.3rem;flex-wrap:wrap;margin:.3rem 0">' + (s.supplied ? '<span class="badge badge-accent">Supplied</span>' : '<span class="badge">Proposed</span>') + ((c.chairs || []).indexOf(s.id) >= 0 ? '<span class="badge badge-ink">Programme chair</span>' : '') + (s.researched ? '<span class="badge badge-ink">Researched · ' + (s.bioSources || []).length + ' source' + ((s.bioSources || []).length === 1 ? '' : 's') + '</span>' : (s.linkedin && s.researchNote ? '<span class="badge">Profile not matched</span>' : '')) + '</span>' + (s.linkedin ? '<a class="small" href="' + esc(s.linkedin) + '" target="_blank" rel="noopener">LinkedIn</a>' : '') + '<span class="small muted" style="display:block;margin-top:.35rem">' + esc(Engine.bioFor(s)) + (s.bioDraft ? ' <em>(draft, pending approval)</em>' : '') + '</span>' + (s.bioSources && s.bioSources.length ? '<span class="small" style="display:block;margin-top:.35rem">Sources: ' + s.bioSources.slice(0, 4).map((u, i) => '<a href="' + esc(u) + '" target="_blank" rel="noopener">[' + (i + 1) + ']</a>').join(' ') + '</span>' : '') + (s.researchNote && !s.researched ? '<span class="small muted" style="display:block;margin-top:.35rem">' + esc(s.researchNote) + '</span>' : '') + '</span></div>').join('') + '</div>' : '<p class="muted">No speakers yet. Sessions show “Speakers to be announced” until you add them.</p>') + '</div>' +
