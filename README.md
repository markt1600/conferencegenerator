...
- **Conference generator** (`/admin/generator`): choose a pre-selected theme (18 themes; six compliance-focused, including *Compliance Officers & Accountability in Financial Institutions*) or define a custom theme, set dates, location and venue, add speakers with optional LinkedIn profiles, then generate:
  ...
  - **Faculty fill**: supply one speaker or twenty. Supplied speakers are kept and given the keynote and panel slots; the rest of the programme is filled to a target size with proposed placeholder profiles (fictional names and organisations, shown as “Invited · confirmation pending” on the site) that you replace as confirmations arrive. Turn it off or change the target in the Speakers step.
  - **Programme chair toggle**: each speaker row has a *Programme chair* switch. Ticked speakers open and close the programme (and host the welcome reception) and are shown as chairs on the site. With nobody ticked, the planner proposes placeholder chairs from the filled faculty, or leaves the opening and closing for chairs to be confirmed when faculty fill is off. When calling the engine directly, set `chair: true` on a speaker, or pass `suppliedChairs: false` to stop the first two supplied speakers chairing by default.
